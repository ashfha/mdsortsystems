...
const themeSets = [["Strand","Entspannung","Sonne"],["Kultur","Essen","Stadt"],["Natur","Abenteuer","Ruhe"],["Luxus","Wellness","Service"],["Nightlife","Strand","Events"],["Familie","Strand","Komfort"]];
const images = ["https://images.unsplash.com/photo-1507525428034-b723cf961d3e","https://images.unsplash.com/photo-1500530855697-b586d89ba3ee","https://images.unsplash.com/photo-1488646953014-85cb44e25828","https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1","https://images.unsplash.com/photo-1501785888041-af3ef285b470","https://images.unsplash.com/photo-1510414842594-a61c69b5ae57"];
...
export const destinations: Destination[] = raw.map(([name,country,region],index)=>{
  ...
  return {id:name.toLowerCase().replace(/[^a-z0-9]+/gi,"-"),name,country,region,themes:themeSets[index%themeSets.length],flightHours:far?9+(index%6):medium?4+(index%5):1.5+(index%4),baseFlight:far?520+(index%8)*55:medium?250+(index%7)*42:90+(index%8)*28,baseHotelNight:58+(index%10)*24,calm:55+(index*7)%45,luxury:50+(index*11)%50,image:`${images[index%images.length]}?auto=format&fit=crop&w=1400&q=82`};
});
...
export function rankDestinations(profile:SearchProfile){
  ...
  const hotelMultiplier=profile.hotelLevel==="Premium"?1.75:profile.hotelLevel==="Comfort"?1.25:.9;
  return destinations.map(destination=>{
    ...
    const reasons=[destination.flightHours<=profile.maxFlight?`Flugzeit passt mit ca. ${destination.flightHours.toFixed(1)} Std.`:"Etwas längere Anreise",total<=profile.budget?"liegt im geplanten Budget":"liegt über dem Zielbudget",`stark bei ${destination.themes.slice(0,2).join(" und ")}`];
