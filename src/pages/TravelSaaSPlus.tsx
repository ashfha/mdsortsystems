...
const HERO_FEATURES = [
  {
    icon: BadgeCheck,
    title: "Was die Website kann",
    text: "Reiseziele verstehen, Flüge und Hotels vergleichen, Budget ausrechnen und einen kompletten Plan bauen.",
  },
  {
    icon: Search,
    title: "Buchen und suchen",
    text: "Oben auf Buchen klicken, alle Daten eintragen, dann mit Suchen sofort Ergebnisse bekommen.",
  },
  {
    icon: Heart,
    title: "Destination wählen",
    text: "Ein Reiseziel öffnen und Hotel-Luxus sowie Kriterien direkt anpassen.",
  },
  {
    icon: Wallet,
    title: "Budget klar sehen",
    text: "Flug, Hotel und Puffer werden zusammen gerechnet, damit du den Gesamtpreis sofort siehst.",
  },
] as const;
...
export default function TravelSaaSPlus() {
  ...
  const previewProfile = useMemo(() => buildProfile(draft), [draft]);
  const activeProfile = useMemo(() => buildProfile(submitted), [submitted]);
  const ranked = useMemo(() => rankDestinations(DESTINATIONS, activeProfile), [activeProfile]);
  const selectedResult = useMemo<RankedDestination | undefined>(() => ranked.find((item) => item.name === selectedName) ?? ranked[0], [ranked, selectedName]);
  const account = useMemo(() => buildAccountPreview(activeProfile, ranked.length), [activeProfile, ranked.length]);
  const liveHooks = useMemo(() => buildLiveHooks(selectedResult, activeProfile), [selectedResult, activeProfile]);
  ...
  function composeAndSearch(nextDraft: Draft = draft, keepSelection = false) {
    setDraft(nextDraft);
    setSubmitted(nextDraft);
    const nextProfile = buildProfile(nextDraft);
    const nextRanked = rankDestinations(DESTINATIONS, nextProfile);
    const preserve = keepSelection && nextRanked.some((item) => item.name === selectedName);
    setSelectedName(preserve ? selectedName : nextRanked[0]?.name ?? DESTINATIONS[0].name);
    setHasSearched(true);
          ...
          <Card className="rounded-[2rem] border-border/70 bg-card p-6 shadow-soft">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <RangeCard label="Max. Flugzeit" value={draft.maxFlightHours} min={2} max={15} step={1} onChange={(value) => setField("maxFlightHours", value)} formatter={(value) => `${value}h`} />
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          {!hasSearched ? (
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-3">
              {topThree.map((item, index) => (
                <ResultCard key={item.name} destination={item} selected={item.name === selectedName} rank={index + 1} onSelect={() => selectDestination(item.name)} />
              ))}
            </div>
          )}
        </section>
...
function buildProfile(draft: Draft) {
  return parseTravelBrief({
...
function RangeCard({ label, value, min, max, step, formatter, onChange }: { label: string; value: number; min: number; max: number; step: number; formatter: (value: number) => string; onChange: (value: number) => void }) {
  return (
    <div className="rounded-3xl border border-border/70 bg-background/55 p-4">
      <div className="flex items-center justify-between gap-3">
...
function previewSummary(profile: ReturnType<typeof buildProfile>) {
  const first = profile.priorities[0] ? `Priority: ${profile.priorities[0]}.` : "Flexible profile.";
  const vibe = profile.vibe === "quiet" ? "Calm trip" : profile.vibe === "lively" ? "Livelier trip" : "Balanced trip";
  return `${first} ${vibe}. Flight target: ${profile.maxFlightHours ? `${profile.maxFlightHours}h max` : "open"}.`;
}
